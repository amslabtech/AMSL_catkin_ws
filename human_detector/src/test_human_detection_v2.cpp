...
sensor_msgs::PointCloud2 position_neg_pt;
pcl::PointCloud<pcl::PointXYZINormal> pcl_org;
pcl::PointCloud<pcl::PointXYZINormal> obj_pt;
pcl::PointCloud<pcl::PointXYZINormal> obj_cen;
////////////////////////////////////////////////
...
int times=0;
const int f_num=329;
svm_model *model=svm_load_model("/home/amsl/AMSL_ros_pkg/human_detection/svm_model/all_feature6_2.model", times);
...
// svm_model *model=svm_load_model("/home/amsl/AMSL_ros_pkg/rwrc15/rwrc15_human_detection/classification/svm_model/all_feature6_2.model", times);
svmRange svm_range;
int svmLoadRange()
{
	...
	// ifstream ifs("/home/amsl/AMSL_ros_pkg/rwrc15/rwrc15_human_detection/classification/svm_model/all_feature6.range");
	string buf;
	int n=0;
...
void visualizeBBox(visualization_msgs::Marker& bbox, pcl::PointCloud<pcl::PointXYZI>& bbox_pt, const int& mode)
{
	...
	bbox.points.resize(bbox_pt.points.size());
	for(size_t i=0;i<bbox_pt.points.size();i++){
		bbox.points[i].x=bbox_pt.points[i].x;
		bbox.points[i].y=bbox_pt.points[i].y;
		bbox.points[i].z=bbox_pt.points[i].z;
	}
}

inline void extractBBoxPt(pcl::PointCloud<pcl::PointXYZI>& bbox_pt, pcl::PointCloud<pcl::PointXYZ>& viz_tmp)
{
	size_t pre_num = bbox_pt.points.size();
	bbox_pt.points.resize(pre_num + 24);
	for(size_t i=0;i<24;i++){
		bbox_pt.points[i+pre_num].x=viz_tmp.points[i].x;
		bbox_pt.points[i+pre_num].y=viz_tmp.points[i].y;
		bbox_pt.points[i+pre_num].z=viz_tmp.points[i].z;
	}
}

inline void extractClusterPt(pcl::PointCloud<pcl::PointXYZINormal>& cluster_pt, clusterInformation& cluster, clusterTemporary& cluster_temp, int& positive_number)
{
	pcl::PointXYZINormal p;
	p.intensity=(float)positive_number;
	p.normal_x=cluster.rsl_rev;
	p.normal_y=cluster.likelihood;
	p.normal_z=cluster.likelihood_flag;
	p.curvature=cluster.track_vel;
	// p.curvature=cluster.pre_id;
	for(size_t i=0;i<cluster_temp.clouds.points.size();i++){
	    p.x=cluster_temp.clouds.points[i].x;
	    p.y=cluster_temp.clouds.points[i].y;
	    p.z=cluster_temp.clouds.points[i].z;
	    p.intensity=cluster_temp.clouds.points[i].intensity;
	    //p.normal_x=cluster_temp[i].clouds.points[i].normal_x;
	    //p.normal_y=cluster_temp[i].clouds.points[i].normal_y;
	    //p.normal_z=cluster_temp[i].clouds.points[i].normal_z;
	    //p.curvature=cluster_temp[i].clouds.points[i].curvature;
		cluster_pt.points.push_back(p);
	}
}

inline void extractBBoxCentroid(pcl::PointCloud<pcl::PointXYZINormal>& bbox_cent, clusterInformation& cluster)
{
	// pcl::PointNormal p;
	pcl::PointXYZINormal p;
	p.x=cluster.centroid[0];
	p.y=cluster.centroid[1];
	p.z=cluster.centroid[2];
	p.normal_x=cluster.width;
	p.normal_y=cluster.length;
	p.normal_z=cluster.max.z;
	bbox_cent.points.push_back(p);
}
...
///////////////////////////////////////////////////////////////////////
vector<clusterTemporary> calFeatures(pcl::PointCloud<pcl::PointXYZINormal>& obj, pcl::PointCloud<pcl::PointXYZINormal>& obj_centroid)
{
...
#pragma omp parallel num_threads((int)obj_centroid.points.size()) reduction(+:c_count,nomp_time) private(start_time,end_time)
// #pragma omp parallel num_threads(32) reduction(+:c_count,nomp_time) private(start_time,end_time)
{
	#pragma omp for nowait
	for(size_t iii=0;iii<obj_centroid.points.size();iii++){
		start_time=omp_get_wtime();
		pcl::PointCloud<pcl::PointXYZINormal>::Ptr cloud_cluster (new pcl::PointCloud<pcl::PointXYZINormal>);
        //caution: "obj_centroid.points[iii].curvature==cluster[iii].points.size()"
        //         "obje_centroid.points[iii].intensity: start_index"
		cloud_cluster->points.resize(obj_centroid.points[iii].curvature);
		for(size_t jjj=0;jjj<obj_centroid.points[iii].curvature;jjj++){
			int p_num=jjj + obj_centroid.points[iii].intensity;
			cloud_cluster->points[jjj]=obj.points[p_num];
			cloud_cluster->width = p_num;
			cloud_cluster->height = 1;
			cloud_cluster->is_dense = false;
		}
		Vector3f centroid_v;
        centroid_v[0]=obj_centroid.points[iii].x;
        centroid_v[1]=obj_centroid.points[iii].y;
        centroid_v[2]=obj_centroid.points[iii].z;
...
{	
	...
	////subscriber////
	ros::Subscriber object_pt_sub = n.subscribe("/cluster/points", 1, ObjectPointCallback);
	ros::Subscriber object_cen_sub = n.subscribe("/cluster/centroid", 1, ObjectCentroidCallback);
	////publisher////
	...
	ros::Publisher negative_position_pt_pub = n.advertise<sensor_msgs::PointCloud2>("/human_recognition/negative_position",1);
	...
	// int total_cluster=0, total_positive=0, total_negative=0;
	...
	while (ros::ok()){
		...
		if(callback_flag){
			...
			// clock_t start=clock();
			vector<clusterTemporary> clusters=calFeatures(obj_pt, obj_cen);
            ...
            pcl::PointCloud<pcl::PointXYZINormal> positive_pcl, negative_pcl;
            pcl::PointCloud<pcl::PointXYZINormal> position_pcl, position_neg_pcl;
            pcl::PointCloud<pcl::PointXYZI> bb_pos_pcl, bb_neg_pcl, bb_div_pcl;

            for(size_t it=0;it<clusters.size();it++){
                int c_num=clusters[it].clouds.points.size();
				if(all_cluster_info[it].object_flag==1){
					...
					extractBBoxPt(bb_pos_pcl, viz_tmp);
					extractBBoxCentroid(position_pcl, all_cluster_info[it]);
					for (int i=0; i<c_num; ++i){
						positive_pcl.points.push_back(clusters[it].clouds.points[i]);
					}
				}
				// else if (all_cluster_info[it].object_flag==0){
                    // pcl::PointCloud<pcl::PointXYZ> viz_tmp;
                    // viz_tmp=visualizer(all_cluster_info[it].rot_mat_inv, all_cluster_info[it].min, all_cluster_info[it].max, all_cluster_info[it].position);
					// extractClusterPt(negative_pcl, all_cluster_info[it], clusters[it], positive_number);
					// extractBBoxPt(bb_neg_pcl, viz_tmp);
					// extractBBoxCentroid(position_neg_pcl, all_cluster_info[it]);
				// }
                // else if(all_cluster_info[it].object_flag==2){
                    // pcl::PointCloud<pcl::PointXYZ> viz_tmp;
                    // viz_tmp=visualizer(all_cluster_info[it].rot_mat_inv, all_cluster_info[it].min, all_cluster_info[it].max, all_cluster_info[it].position);
					// extractBBoxPt(bb_div_pcl, viz_tmp);
					// extractBBoxCentroid(position_neg_pcl, all_cluster_info[it]);
                // }
			...
			//std::cout<<"Num of human:"<<position_pcl.points.size()<<std::endl;
			...
			// pcl::toROSMsg(position_neg_pcl,position_neg_pt);
			// position_neg_pcl.points.clear();
			// position_neg_pt.header.frame_id="/velodyne";
			// position_neg_pt.header.stamp=ros::Time::now();
			// negative_position_pt_pub.publish(position_neg_pt);
			//visualize bounding boxes//
            visualization_msgs::Marker bb_pos;
			visualizeBBox(bb_pos, bb_pos_pcl, 1);
            bb_pos_pub.publish(bb_pos);
            bb_pos_pcl.points.clear();

            // visualization_msgs::Marker bb_neg;
			// visualizeBBox(bb_neg, bb_neg_pcl, 0);
            // bb_neg_pub.publish(bb_neg);
            // bb_neg_pcl.points.clear();



			//visualize likelihood in rviz//
            ...
            // test.color.b=1.0;
            // test.color.a=1.0;
			// test_all.markers.resize(all_cluster_info.size());
