...
{	
	...
	if(MODE==0)original_sub = n.subscribe("/sq_lidar/points/lcl", 1, originalCallback);
	...
	while (ros::ok()){
		...
		if(callback_flag){
			...
			original_pt.header.frame_id="/center_laser_";
			...
			removed_pt.header.frame_id="/center_laser_";
